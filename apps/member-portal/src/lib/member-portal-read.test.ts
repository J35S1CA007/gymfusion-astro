...
const responseFor = (view: string, part3: unknown = { completed: false }, extraData: Record<string, unknown> = {}) => Response.json({
  ...
  ok: true,
  view,
  data: view === "eoi" || view === "dashboard" ? { ...extraData, part3 } : extraData,
});
...
test("accepts valid incomplete, complete, and no-active member reads", async () => {
  for (const upstream of [
    response(),
    response(completePart3),
    response({ completed: false }, { state: "no_active_episode" }),
  ]) {
...
test("rejects malformed authenticated success, wrong view, 401, 503, and invalid Part 3", async () => {
  const malformed = [
...
test("accepts Dashboard and established non-EOI view contracts without Part 3", async () => {
  ...
  const responses = [
    ...
    ["supporting-evidence", { items: [], state: "empty" }],
    ["submissions", { items: [], state: "empty" }],
    ["profile-change-requests", { items: [], state: "empty" }],
  ] as const;
  for (const [view, data] of responses) {
    const result = await readMemberPortalView(request(), "https://login.example", view, async () => responseFor(view, undefined, data));
    assert.equal(result.kind, "authenticated", view);
    if (result.kind === "authenticated") assert.deepEqual(result.data, data);
  }

  const malformed = await readMemberPortalView(request(), "https://login.example", "dashboard", async () => responseFor("dashboard", { completed: "true" }));
  assert.deepEqual(malformed, { kind: "unavailable" });
});

test("rejects the identified malformed Part 3 shapes at the Portal trust boundary", async () => {
  ...
  const malformedParts: Array<[string, unknown]> = [
    ...
    ["mixed no_issues selection", { ...completePart3, answers: { ...completePart3.answers, injuries: ["no_issues", "current_injury"], injuryDetails: "A current limitation that affects safe training and needs to be considered." } }],
