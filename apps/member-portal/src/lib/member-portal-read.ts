...
function validateDashboardData(value: Record<string, unknown>): Record<string, unknown> | null {
  if (!isPlainObject(value.onboarding) && !("part3" in value)) return null;
  if ("part3" in value) {
    const part3 = validateDashboardPart3(value.part3);
...
function validateEoiData(value: Record<string, unknown>): Record<string, unknown> | null {
  if (!("parts" in value) && !("part3" in value)) return null;
  if ("parts" in value) {
    if (!Array.isArray(value.parts) || !value.parts.every(isPlainObject)) return null;
    for (const part of value.parts) {
      if (!["2", "3", "4"].includes(String(part.part)) || typeof part.completed !== "boolean") return null;
    }
  }
  if (!("part3" in value)) return value;
  const part3 = validatePart3(value.part3);
  return part3 ? { ...value, part3 } : null;
}
...
function validateReadPayload(value: unknown, expectedView: string): { view: string; data: Record<string, unknown> } | null {
  ...
  if (expectedView === "eoi") {
    data = validateEoiData(value.data);
  } else if (expectedView === "dashboard") {
    data = validateDashboardData(value.data);
